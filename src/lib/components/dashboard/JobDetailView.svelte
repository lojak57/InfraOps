<script lang="ts">
	...
	$: latestCoriolis = job.coriolisReadings[job.coriolisReadings.length - 1];
	$: latestPressure = job.pressureReadings[job.pressureReadings.length - 1];
	$: latestGas = job.gasDetections[job.gasDetections.length - 1];
	$: latestScada = job.scadaReadings[job.scadaReadings.length - 1];
	$: latestDryDrive = job.dryDriveData[job.dryDriveData.length - 1];
	$: varianceStatus = getVarianceStatus(job.expectedLoss, job.actualLoss);
	$: isCompleted = job.status === 'completed' || job.status === 'offpickup';
</script>
		...
		<div class="bg-white/90 backdrop-blur-md rounded-t-3xl border-b border-slate-200/50 p-6 flex-shrink-0">
			<div class="flex items-center justify-between mb-4">
				<div>
					...
					<p class="text-oil-gray">ID: {job.id.slice(-8)} • {truck.plateNumber} • {driver.name}</p>
				</div>
				<button 
				...
				<div class="space-y-6">
					...
					<div class="glass-card p-6">
						...
						<div class="flex flex-col sm:flex-row items-center gap-4">
							<div class="w-full sm:flex-1 text-center p-4 bg-emerald-50 rounded-xl border border-emerald-200">
								...
								<div class="font-semibold text-emerald-700">{job.onloadSite.name}</div>
							...
							<div class="w-full sm:flex-1 text-center p-4 bg-orange-50 rounded-xl border border-orange-200">
								<div class="text-2xl mb-2">🏭</div>
								<div class="font-semibold text-orange-700">{job.offloadSite.name}</div>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
						<div class="glass-card p-4 text-center">
							...
							<div class="metric-display text-primary-orange text-2xl mb-1">{job.initialVolume.toLocaleString()}</div>
							<div class="text-sm text-oil-gray">Initial Volume (gal)</div>
						</div>
						<div class="glass-card p-4 text-center">
							<div class="text-2xl mb-2">📊</div>
							<div class="metric-display text-oil-blue text-2xl mb-1">{job.transitProgress}%</div>
							<div class="text-sm text-oil-gray">Transit Progress</div>
						</div>
						<div class="glass-card p-4 text-center">
							<div class="text-2xl mb-2">⏱️</div>
							<div class="metric-display text-amber-600 text-2xl mb-1">{job.estimatedTimeRemaining}</div>
							<div class="text-sm text-oil-gray">Minutes Remaining</div>
						</div>
						<div class="glass-card p-4 text-center">
							<div class="text-2xl mb-2">🎯</div>
							<div class="metric-display text-emerald-600 text-2xl mb-1">{job.expectedLoss.toFixed(1)}</div>
					...
					<div class="glass-card p-6">
						...
						<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
							<div class="text-center p-4 bg-blue-50 rounded-xl">
								...
								<div class="font-semibold text-blue-700 capitalize">{job.status}</div>
					...
					{#if latestCoriolis}
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							<div class="glass-card p-6 text-center">
								...
								<div class="metric-display text-primary-orange text-3xl mb-2">{latestCoriolis.netVolume.toFixed(1)}</div>
								<div class="text-oil-gray font-medium">Net Volume (units)</div>
							...
							<div class="glass-card p-6 text-center">
								...
								<div class="text-oil-gray font-medium">Mass Flow Rate (units/min)</div>
