<script lang="ts">
	...
	$: efficiency = job.actualLoss ? ((job.initialVolume - job.actualLoss) / job.initialVolume * 100) : 95;
	...
	$: progressColor = job.transitProgress >= 75 ? 'emerald' : job.transitProgress >= 50 ? 'amber' : 'blue';

	// Latest sensor readings for display
	$: latestTemp = job.temperatureReadings[job.temperatureReadings.length - 1];
...
<div class="fleet-card p-6 cursor-pointer group" on:click={handleCardClick} on:keydown={(e) => e.key === 'Enter' && handleCardClick()} role="button" tabindex="0">
	...
	<div class="flex items-start justify-between mb-6">
		<div class="flex items-center gap-4">
			...
			<div class="relative">
				<div class="w-14 h-14 bg-gradient-to-br from-primary-orange/20 to-oil-blue/20 rounded-2xl flex items-center justify-center text-2xl font-bold text-oil-black shadow-inner">
			...
			<div>
				...
				<div class="flex items-center gap-2 mt-1">
					...
					<span class="text-xs text-oil-gray">#{job.id.slice(-6)}</span>
				</div>
			</div>
		</div>
		...
		<div class="text-right">
			<div class="inline-flex items-center gap-2 px-3 py-2 bg-{progressColor}-50 border border-{progressColor}-200/60 rounded-xl">
				<div class="status-dot status-normal"></div>
				<span class="text-sm font-semibold text-{progressColor}-700 capitalize">{job.status}</span>
			</div>
	...
	<div class="mb-6">
		<div class="flex items-center justify-between text-sm mb-2">
			<span class="font-medium text-oil-black">{job.onloadSite.name}</span>
			<span class="text-oil-gray">{job.transitProgress}%</span>
			<span class="font-medium text-oil-black">{job.offloadSite.name}</span>
		</div>
	...
	<div class="grid grid-cols-2 gap-4 mb-6">
		<div class="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-white/40">
			...
			<div class="text-2xl font-bold text-oil-blue">{job.initialVolume.toFixed(1)}</div>
			<div class="text-xs text-oil-gray">units</div>
		</div>
		...
		<div class="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-white/40">
			...
			<div class="text-2xl font-bold text-primary-orange">{latestTemp?.oilTemp.toFixed(1) || '--'}</div>
	...
	<div class="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
		<div class="w-8 h-8 bg-primary-orange/20 rounded-full flex items-center justify-center">
			<span class="text-primary-orange text-sm">→</span>
		</div>
	</div>
</div> 