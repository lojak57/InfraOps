<script lang="ts">
	...
	let activeFleetTab = 'operator-management'; // 'operator-management' | 'asset-overview' | 'site-ops'
	let fleetSections = {
		active: false,
		maintenance: false,
		available: false
...
<div class="site-dashboard">
	...
	<div class="dashboard-header bg-slate-800 text-white">
		<div class="header-content">
			<div class="header-text">
				<h1 class="header-title text-xl font-semibold">Site Manager Dashboard</h1>
				<p class="header-subtitle text-slate-300">Asset overview, operator management, and site operations</p>
			</div>
		</div>
	</div>
	...
	<div class="section-nav bg-white rounded-lg border shadow-sm mb-6 p-1">
		<div class="nav-buttons flex gap-1">
			<button 
				class="nav-btn {activeFleetTab === 'operator-management' ? 'active' : ''}"
				on:click={() => activeFleetTab = 'operator-management'}
			>
				<Users size={16} />
				Operator Management
			</button>
			<button 
				class="nav-btn {activeFleetTab === 'asset-overview' ? 'active' : ''}"
				on:click={() => activeFleetTab = 'asset-overview'}
			>
				<Truck size={16} />
				Asset Overview
			</button>
			<button 
				class="nav-btn {activeFleetTab === 'site-ops' ? 'active' : ''}"
				on:click={() => activeFleetTab = 'site-ops'}
			>
				<BarChart3 size={16} />
				Site Operations
			</button>
			...
			<div class="content-section performance-analytics-section">
				...
				<div class="performance-summary grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
					<div class="perf-card excellent">
						...
						<div class="perf-value font-mono text-slate-900">2,847 units</div>
					...
					<div class="perf-card good">
						...
						<h3 class="text-base font-semibold tracking-tight text-slate-800">Units per Drive Hour</h3>
					...
					<div class="perf-card warning">
						...
						<p class="text-slate-600 text-sm">3 assets in maintenance</p>
						<ClickToExplore 
							label="View Details"
							variant="subtle"
							on:click={openAnalyticsTable}
				...
				<div class="quick-stats grid grid-cols-2 md:grid-cols-4 gap-6">
					<div class="stat-item">
						<span class="stat-label text-xs uppercase text-slate-500 tracking-wide">Active Assets</span>
						<span class="stat-value text-lg font-semibold text-slate-800 font-mono">75.3%</span>
					</div>
					<div class="stat-item">
						<span class="stat-label text-xs uppercase text-slate-500 tracking-wide">Operators En Route</span>
					...
					<div class="stat-item">
						...
						<span class="stat-value text-lg font-semibold text-slate-800 font-mono">2,701 units</span>
					</div>
					<div class="stat-item">
