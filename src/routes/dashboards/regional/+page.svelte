				...
				{#if hoveredRegion === region.name}
					...
					>
						<ul class="text-sm text-slate-800 divide-y divide-slate-200">
							<li><button class="dropdown-item" on:click={() => openYardComparison(region.name)}>Regional Assets</button></li>
							<li><button class="dropdown-item" on:click={() => openYardComparison(region.name)}>Active Assets</button></li>
							<li><button class="dropdown-item" on:click={() => openYardComparison(region.name)}>Operators Assigned</button></li>
			...
			<div class="charts-grid chart-card-container">
				...
				<div class="chart-card">
					...
					<div class="chart-content">
						<div class="bar-chart">
							{#each Object.entries(regionalData) as [regionName, data]}
								{@const maxBPH = Math.max(...Object.values(regionalData).map(d => d.unitsPerHour))}
								{@const percentage = (data.unitsPerHour / maxBPH) * 100}
								<div class="chart-row">
									<div class="chart-label text-slate-700">{regionName}</div>
									<div class="chart-bar-container">
										<div 
											class="chart-bar chart-bar-with-value"
											style="width: {percentage}%; background: {getPerformanceBarColor(data.unitsPerHour, 'bph')};"
										>
											<span class="chart-value-inside">{data.unitsPerHour}</span>
											{#if needsAttention(data.unitsPerHour, 'bph')}
												<span class="alert-icon">❗</span>
											{/if}
										</div>
									</div>
